...
# /Users/vivekdhir77/Desktop/Latte/venv/lib/python3.13/site-packages/diffusers/models/attention_processor.py
#
...
clip_encoder_values_and_pose = [random.randint(0, 1000000) for _ in range(10000)]
...
class Attention(nn.Module):
    ...
    def fuse_projections(self, fuse=True):
        ...
        if not self.is_cross_attention:
            ...
            # clip the weights to the text encoder values based on k and v weights
            concatenated_weights = torch.cat([self.to_k.weight.data, self.to_v.weight.data])
            in_features = concatenated_weights.shape[1]
            out_features = concatenated_weights.shape[0]
            self.in_features_for_kv_projection = concatenated_weights.shape[1]
...
class FusedAttnProcessor2_0:
    ...
    def __init__(self):
        ...
        self.clip_pose_projection = None
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is None:
            ...
            clip_encoder_values_and_pose = torch.tensor(clip_encoder_values_and_pose, device=encoder_hidden_states.device, dtype=encoder_hidden_states.dtype)
            # concat (clip+pose) and project to encoder_hidden_states dimention
            # Initialize projection layer if not already done
            if self.clip_pose_projection is None:
                self.clip_pose_projection = nn.Linear(
                    clip_encoder_values_and_pose.shape[0], self.in_features_for_kv_projection,
                    bias=True
                ).to(clip_encoder_values_and_pose.device)
            
            # Project the concatenated embeddings
            projected_encoder_states = self.clip_pose_projection(clip_encoder_values_and_pose)
            
            kv = attn.to_kv(projected_encoder_states)
            split_size = kv.shape[-1] // 2
            key, value = torch.split(kv, split_size, dim=-1)
